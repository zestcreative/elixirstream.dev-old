defmodule ElixirStream.Twitter do
  ...
  def publish(tip) do
    with {:ok, tip, file} <- Catalog.generate_codeshot(tip),
         {:ok, media_id} <- Client.upload_media(file, filename: url_safe(tip.title) <> Path.extname(file)),
         {:ok, %{body: %{"id_str" => twitter_status_id}}} <- Client.update_status(tweet_body(tip), [media_id]),
